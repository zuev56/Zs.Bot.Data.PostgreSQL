...
public sealed class QueryFactory : IQueryFactory
{
    ...
    {
        var (path, value, _) = condition;
        var pgJsonPath = path.ToPostgresqlJsonPath();
        var pgRawDataJsonPath = pgJsonPath.ToRawDataColumnJsonPath(value);
        var sqlCondition = ConvertToSqlCondition(condition);
        return $"{pgRawDataJsonPath} {sqlCondition}";
    }
